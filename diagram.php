...
<body class="animsition">
	...
	//doughut chart
    <?php 
    $if = mysqli_query($konek, "SELECT * FROM mahasiswa WHERE jurusan = 'Teknik Informatika'");
    $ms = mysqli_query($konek, "SELECT * FROM mahasiswa WHERE jurusan = 'Teknik Mesin'");
    $sp = mysqli_query($konek, "SELECT * FROM mahasiswa WHERE jurusan = 'Teknik Sipil'");
    $el = mysqli_query($konek, "SELECT * FROM mahasiswa WHERE jurusan = 'Teknik Elektro'");

    $jml_if = mysqli_num_rows($if);
    $jml_ms = mysqli_num_rows($ms);
    $jml_sp = mysqli_num_rows($sp);
    $jml_el = mysqli_num_rows($el);

    $produk = mysqli_query($konek, "SELECT jurusan, COUNT(*) jumlah FROM mahasiswa GROUP BY jurusan HAVING COUNT(jumlah)  > 0");
